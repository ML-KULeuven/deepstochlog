...
from examples.citeseer.with_influence.citeseer_data_withinfluence import train_dataset, valid_dataset, test_dataset, queries_for_model, citations
from examples.citeseer.citeseer_utils import create_model_accuracy_calculator, Classifier, RuleWeights, Influence
...
def run(
    ...
    influence = Influence(num_documents=len(train_dataset.documents))
    classifier_network = Network(
        "classifier",
        classifier,
        index_list=[Term("class"+str(i)) for i in range(6)],
    )
    rule_weights_network = Network(
        "rule_weight",
        rule_weights,
        index_list=[Term(str("neural")), Term(str("cite"))],
    )
    influence_network = Network(
        "influence",
        influence,
        index_list=[Term(str(1))],
    )
    networks = NetworkStore(classifier_network, rule_weights_network, influence_network)
    ...
    model = DeepStochLogModel.from_file(
        file_location=str((root_path / "citeseer_influence.pl").absolute()),
    ...
    # )
    calculate_model_accuracy =  create_model_accuracy_calculator( model, test_dataset, time() )
