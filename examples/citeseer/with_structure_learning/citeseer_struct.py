...
from examples.citeseer.with_structure_learning.citeseer_data_struct import train_dataset, valid_dataset, test_dataset, queries_for_model, citations, pretraining_data
from examples.citeseer.citeseer_utils import create_model_accuracy_calculator, Classifier, RuleWeights, AccuracyCalculator, pretraining
...
def run(
    ...
    rule_weight = Network(
        "rule_weight",
        rule_weights,
        index_list=[Term(str("neural")), Term(str("citep"))],
    )
    networks = NetworkStore(classifier_network, rule_weight)

    # x, y = pretraining_data
    # pretraining(x, y, classifier, optimizer, epochs=300)
    ...
    model = DeepStochLogModel.from_file(
        file_location=str((root_path / "citeseer_struct.pl").absolute()),
    ...
    calculate_model_accuracy = AccuracyCalculator( model=model,
                                                    valid=valid_dataset,
                                                    test=test_dataset,
                                                    start_time=time(),after_epoch=30)
