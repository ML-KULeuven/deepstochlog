...
from tabled_tree_builder import TabledAndOrTreeBuilder
...
def run(
    ...
    query = create_expression_sentence_query(number_img=max_length, total_sum="_")
    print(query)

    proving_start = time()
    file_location = str((root_path / "mathexpression_notabling.pl").absolute())
    with open(file_location) as f:
        lines = f.readlines()
    program = "\n".join(lines)
    builder = TabledAndOrTreeBuilder(
        parse_rules(program), verbose=False, tabling=tabling
    )
    program = builder._prolog_only(queries=query)
...
if __name__ == "__main__":
    ...
    copy2("mathexpression_notabling.pl", logs)
    ...
    for seed in [0]:
        ...
        for l, tabling in product([11], [True, False]):
            print("Length= %d, Tabling=%s" % (l, str(tabling)))
