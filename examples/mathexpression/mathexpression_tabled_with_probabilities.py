...
from nn_models import TrainableProbability
...
def load_expression_networks(lr=1e-3):
    ...
    term_network = Network(
        "term",
        TrainableProbability(N=3),
        index_list=[Term(str(i)) for i in range(3)],
    )

    expression_network = Network(
        "expression",
        TrainableProbability(N=3),
        index_list=[Term(str(i)) for i in range(3)],
    )

    return NetworkStore(number_network, operator_network, term_network, expression_network)
...
def run(
    ...
    model = DeepStochLogModel.from_file(
        file_location=str((root_path / "mathexpression_with_probabilities.pl").absolute()),
        query=query,
        networks=networks,
        device=device,
    )
...
if __name__ == "__main__":
    ...
    copy2("mathexpression_with_probabilities.pl", logs)
